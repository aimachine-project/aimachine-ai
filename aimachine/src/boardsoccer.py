...
class BoardSoccer:
    ...
    def __init__(self):
        ...
        for it in range(0, self.gateHalfWidth):
            self.nodes[0][self.middleColIndex - it].make_link(NodeLink.LINK_LEFT)
            self.nodes[0][self.middleColIndex + it].make_link(NodeLink.LINK_RIGHT)
            self.nodes[self._BOARD_HEIGHT][self.middleColIndex - it].make_link(NodeLink.LINK_LEFT)
            self.nodes[self._BOARD_HEIGHT][self.middleColIndex + it].make_link(NodeLink.LINK_RIGHT)
        # link    gate    skews
        ...
        self.nodes[self._BOARD_HEIGHT - 1][self.middleColIndex - self.gateHalfWidth].make_link(NodeLink.LINK_TOP_LEFT)
        self.nodes[self._BOARD_HEIGHT - 1][self.middleColIndex + self.gateHalfWidth].make_link(NodeLink.LINK_TOP_RIGHT)
        ...
        for it in range(1, self._BOARD_HEIGHT):
            left_border_node = self.nodes[it][1]
            ...
            right_border_node = self.nodes[it][self._BOARD_WIDTH - 1]
        ...
        for it in range(1, self.middleColIndex - self.gateHalfWidth):
            top_left_border_node = self.nodes[1][it]
            ...
            top_right_border_node = self.nodes[1][self._BOARD_WIDTH - it]
            ...
            bottom_left_node = self.nodes[self._BOARD_HEIGHT - 1][it]
            ...
            bottom_right_node = self.nodes[self._BOARD_HEIGHT - 1][self._BOARD_WIDTH - it]
...
class Node:
    ...
    def has_link(self, link: NodeLink) -> bool:
        return self._links.__contains__(link)

    def has_any_free_link(self) -> bool:
        return len(self._links) < len(NodeLink)
    ...
    def make_link(self, link: NodeLink):
        if link is NodeLink.LINK_TOP:
            ...
            linked_node._links.append(NodeLink.LINK_BOTTOM)
            return linked_node
        elif link is NodeLink.LINK_TOP_RIGHT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index - 1][self.col_index + 1]
            linked_node._links.append(NodeLink.LINK_BOTTOM_LEFT)
            return linked_node
        elif link is NodeLink.LINK_RIGHT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index][self.col_index + 1]
            linked_node._links.append(NodeLink.LINK_LEFT)
            return linked_node
        elif link is NodeLink.LINK_BOTTOM_RIGHT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index + 1][self.col_index + 1]
            linked_node._links.append(NodeLink.LINK_TOP_LEFT)
            return linked_node
        elif link is NodeLink.LINK_BOTTOM:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index + 1][self.col_index]
            linked_node._links.append(NodeLink.LINK_TOP)
            return linked_node
        elif link is NodeLink.LINK_BOTTOM_LEFT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index + 1][self.col_index - 1]
            linked_node._links.append(NodeLink.LINK_TOP_RIGHT)
            return linked_node
        elif link is NodeLink.LINK_LEFT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index][self.col_index - 1]
            linked_node._links.append(NodeLink.LINK_RIGHT)
            return linked_node
        elif link is NodeLink.LINK_TOP_LEFT:
            self._links.append(link)
            linked_node = self._board.nodes[self.row_index - 1][self.col_index - 1]
            linked_node._links.append(NodeLink.LINK_BOTTOM_RIGHT)
            return linked_node
        else:
